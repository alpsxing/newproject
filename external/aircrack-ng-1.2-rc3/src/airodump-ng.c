...
int dump_add_packet( unsigned char *h80211, int caplen, struct rx_info *ri, int cardnum,void (*get_scan_record)(void *), void *rec )
//int dump_add_packet( unsigned char *h80211, int caplen, struct rx_info *ri, int cardnum)
{
    ...
    {
        tmptime = (int)ap_cur->tlast;
        tmppower = ap_cur->power_lvl[ap_cur->power_index];
        ...
        ( ( h80211[1] & 3 ) == 2 ) )
    {
        ...
        if( n > 0 )
        {
            ...
            if(tmptime != ap_cur->tlast || tmppower != ri->ri_power)
            {
                memcpy(mrec.mac, bssid, 6);
                mrec.type = 2;
                mrec.max_signal = ap_cur->best_power;
                mrec.latest_signal = ap_cur->power_lvl[ap_cur->power_index];
                mrec.time = ap_cur->tlast;
                get_scan_record(&mrec);
            }
        }
        else
            ap_cur->avg_power = -1;
...
//         if(ap_cur->fcapt >= QLT_COUNT) update_rx_quality();
    ...
    {
        tmptime = st_cur->tlast;
        tmppower = st_cur->power;
        /* update the last time seen */
	...
	if(ri->ri_channel > 0 && ri->ri_channel <= HIGHEST_CHANNEL)
        ...
        if(tmptime != st_cur->tlast || tmppower != ri->ri_power)
        {
            memcpy(mrec.mac, stmac, 6);
            mrec.type = 1;
            mrec.max_signal = st_cur->best_power;
            mrec.latest_signal = st_cur->power;
            mrec.time = st_cur->tlast;
            get_scan_record(&mrec);    
        }
    ...
    if(caplen < 24 && caplen >= 10 && h80211[0])
    {
        ...
        if(h80211[0] & 0x04)
        {
            ...
            while(p <= h80211+16 && p<=h80211+caplen)
            {
                ...
                {
                    tmptime = na_cur->tlast;
                    tmppower = na_cur->power;
                ...
                if(ri->ri_power > na_cur->best_power)
                    na_cur->best_power = ri->ri_power;
                ...
                if(tmptime != na_cur->tlast || tmppower != ri->ri_power)
                {
                    memcpy(mrec.mac, namac, 6);
                    mrec.type = 0;
                    mrec.max_signal = na_cur->best_power;
                    mrec.latest_signal = na_cur->power;
                    mrec.time = na_cur->tlast;
                    get_scan_record(&mrec);
                }
...
int dump_write_csv( void )
{
    ...
    while( na_cur != NULL )
    {
        ...
        fprintf( G.f_txt, "  %3d", na_cur->channel  );
        fprintf( G.f_txt, " %3d", na_cur->power  );
        fprintf( G.f_txt, " %6d", na_cur->ack );
        fprintf( G.f_txt, "  %4d", na_cur->ackps );
        fprintf( G.f_txt, " %6d", na_cur->cts );
        fprintf( G.f_txt, " %6d", na_cur->rts_r );
        fprintf( G.f_txt, " %6d", na_cur->rts_t );
        fprintf( G.f_txt, " %6d", na_cur->other );
...
int airodum_scan_main(char *s_iface, void (*get_scan_record)(void *), void *rec)
{
    long time_slept, cycle_time, cycle_time2;
    int caplen=0, i, fdh, fd_is_set, chan_count, freq_count, unused;
    int fd_raw[MAX_CARDS], arptype[MAX_CARDS];
    int ivs_only;
    int valid_channel;
    char ifnam[64];
...
//                         return 1;
                    ...
                    wi_read_failed = 0;
                    dump_add_packet( h80211, caplen, &ri, i,get_scan_record,rec);
                }
            }
        }
    }
